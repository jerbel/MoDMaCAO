...
import org.modmacao.metric.Metric;
 ...
 * http://schemas.modmacao.org/occi/platform# - term: component - title:
 * Component
 */
public class ComponentConnector extends org.modmacao.occi.platform.impl.ComponentImpl {
	...
	public void occiCreate() {
		utils = new Utils(this.getId());
		utils.getPredictedValue();
		Metric m = utils.getMetric();
		this.getParts().add(m);
	...
	public void undeploy() {
		...
		case Status.ACTIVE_VALUE:
			...
			// TODO: Other components that have an execution dependency TO this component
			// should be stopped.
			this.stop();
		...
		utils.getActualValues(StatusType.UNDEPLOY);
		Metric m = utils.getMetric();
		this.getParts().add(m);
	}
	...
	public void stop() {
		...
		utils.getActualValues(StatusType.STOP);
		Metric m = utils.getMetric();
		this.getParts().add(m);
	}
	...
	public void start() {
		...
		case Status.INACTIVE_VALUE:
			...
			if (status == 0 && assertCompsStatusEquals(getExecutionDependendComps(), Status.ACTIVE))
				...
				//TODO: not implmented yet, just figure out which condition we should add
				}
		...
		utils.getActualValues(StatusType.START);
		Metric m = utils.getMetric();
		this.getParts().add(m);
	}
	...
	public void configure() {
		...
		utils.getActualValues(StatusType.CONFIGURE);
		Metric m = utils.getMetric();
		this.getParts().add(m);
	}
	...
	public void deploy() {
		...
		utils.getActualValues(StatusType.DEPLOY);
		Metric m = utils.getMetric();
		this.getParts().add(m);
	}
	// End of user code
